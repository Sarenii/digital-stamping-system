...
const StampCreator = ({ createStamp }) => {
  ...
  const { user } = useAuth();
  const token = user?.accessToken || localStorage.getItem("authToken"); // Get the token from context or localStorage
  ...
  const handleCreateStamp = async () => {
    ...
    const token = user?.accessToken || localStorage.getItem("authToken");
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/stamps/",
        stampData,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Make sure the token is added in the header
          },
        }
      );
